...
export class TelegramService {
  ...
  /**
   * Правит текст (или подпись) уже опубликованного поста вместе с кнопкой —
   * этим публикуются итоги конкурса.
   *
   * Отличие от updateContestPublishedMessage: медиа НЕ трогаем. Там ради смены
   * текста дёргается editMessageMedia, то есть картинка перезаливается на
   * серверы Telegram заново; здесь меняется только подпись.
   *
   * hasPhoto приходит из payload публикации (что реально отправляли). Если он
   * всё же разошёлся с фактом, Telegram ответит «нет текста/подписи для правки» —
   * пробуем противоположный метод, иначе пост молча остался бы без итогов.
   */
  async updateContestResultsMessage(dto: {
    chatId: string;
    messageId: number;
    text: string;
    buttonText: string;
    ...
    try {
      try {
        await (dto.hasPhoto ? editCaption() : editText());
      } catch (error: any) {
        const code = error?.response?.error_code;
        const description = error?.response?.description;

        // Единственный случай, который лечится повтором другим методом:
        // сообщение оказалось не того типа, что записано в payload. Всё
        // остальное (в том числе «не изменено») уходит во внешний catch.
        const wrongMessageKind =
          code === 400 &&
          typeof description === 'string' &&
        ...
        if (!wrongMessageKind) throw error;
        ...
        this.logger.warn(
          ...
          'updateContestResultsMessage: тип сообщения разошёлся с payload, пробуем другой метод',
        );

        await (dto.hasPhoto ? editText() : editCaption());
      }
    } catch (error: any) {
      const code = error?.response?.error_code;
      const description = error?.response?.description;

      // Пост уже такой же / удалён / нередактируем — повторять бессмысленно.
      if (
        code === 400 &&
        typeof description === 'string' &&
        (description.includes('message is not modified') ||
