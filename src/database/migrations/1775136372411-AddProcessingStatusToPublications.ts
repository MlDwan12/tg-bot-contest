...
export class AddProcessingStatusToPublications1775136372411 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TYPE "public"."contest_publications_status_enum" RENAME TO "contest_publications_status_enum_old"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."contest_publications_status_enum" AS ENUM('pending', 'processing', 'published', 'failed', 'cancelled')`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."contest_publications_status_enum_old" AS ENUM('pending', 'processing', 'published', 'failed')`,
    );
    await queryRunner.query(
      `ALTER TABLE "contest_publications" ALTER COLUMN "status" DROP DEFAULT`,
    );
