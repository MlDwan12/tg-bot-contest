...
- **⏭️ ТОЧНЫЙ СЛЕДУЮЩИЙ ШАГ (продолжить отсюда): Фаза 9 В ПРОЦЕССЕ — единый паттерн репо (токен+интерфейс). User СДЕЛАН как шаблон, дальше Channel.** Эталон (память [[repository-pattern-convention]]): один `XRepository implements IXRepository` + один `IXRepository` + токен `X_REPOSITORY` (`tokens.ts`) + провайдер `{ provide, useClass }` + инъекция `@Inject(X_REPOSITORY) repo: IXRepository` (интерфейс — `import type`!). **Осталось слить read/write у 4 агрегатов (уже на токенах, но раздвоены):** Channel (проще, 1 инъектор) → Contest → ContestParticipate → ContestWinner. Схема на каждый: (1) слить 2 репо-класса → `x.repository.ts`; (2) слить 2 интерфейса → `x-repository.interface.ts` по факту реализации; (3) 2 токена → 1 `X_REPOSITORY` в tokens.ts; (4) провайдер один; (5) обновить инъекции в сервисах (+ spec-моки); (6) удалить старые файлы, barrels. Правило шага: build чист + core 18/18 + boot 1/1. **Метод тестов:** `yarn test:db:up` если контейнеры погашены, затем `yarn test:int` (= core 18 + boot 1, разные процессы). **НЕ закоммичен:** User-шаблон (шаг 1 Ф9).
...
- [~] Фаза 9 — схлопнуть CQRS-репозитории 🟢 (В ПРОЦЕССЕ: эталон токен+интерфейс выбран, User слит; осталось Channel/Contest/Participate/Winner)
