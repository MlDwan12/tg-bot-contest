...
- **⏭️ ТОЧНЫЙ СЛЕДУЮЩИЙ ШАГ (продолжить отсюда): Фаза 6 В ПРОЦЕССЕ — сеть (bootstrap-smoke) ГОТОВА, дальше резать первый цикл.** Сеть: `bootstrap.int-spec.ts` реально поднимает `AppModule.compile()` (сторож против битых `forwardRef`). Прогон: `yarn test:int` = core 18/18 + boot 1/1 (изолированы в разные процессы — иначе BullMQ-воркеры bootstrap флакуют F1-гонку). **✅ Цикл users↔auth РАЗОРВАН** (шаг 2): AuthModule `@Global` → избыточные `forwardRef(AuthModule)` в users/channels убраны, auth→users стал обычным импортом. core 18/18 + boot 1/1. **Осталось 3 цикла:** contests↔contests-jobs, bot↔channels, толстый bot↔contests (хаб TelegramService — последним). Приём: искать избыточность (@Global/односторонняя реальная зависимость), резать, проверять boot. Правило шага: после разрыва И core 18/18, И boot 1/1. **Не закоммичено:** net Фазы 6 (шаг 1) + разрез users↔auth (шаг 2) — закоммитить.
