...
- **Статус:** **Фазы 0 и 2 ЗАКРЫТЫ.** Сеть безопасности + фикс F1 готовы: **16 тестов, 8 сьютов, ВСЕ зелёные, exit 0.** Покрыты 3 горячих пути (участие/идемпотентность/розыгрыш+reuse-guard); F1-гонка закрыта воротами `updateStatusIfNotCompleted` (+ регрессионный сторож `winner-draw-race`); пустой конкурс штатно завершается (0 победителей).
...
- **⏭️ ТОЧНЫЙ СЛЕДУЮЩИЙ ШАГ (продолжить отсюда): Фаза 3 — Аудит (F3 + Q5.2).** Приоритет пользователя №2:
  1. **Crypto-rng:** заменить `Math.random()` в `shuffleArray` (`contest-winner.service.ts:254`) на `crypto.randomInt`. Для дорогих RANDOM (до 10М ₽) не-крипто-рандом недопустим. Правило + 🟢/🔴 перед тестом.
  2. **След розыгрыша** (seed/входные данные) для доказуемости — решить, куда сохранять (новая таблица `draw_audit`?). Обсудить с пользователем схему.
  3. **Аудит-лог MANUAL-назначения** победителя (кто/когда/кого) — защита от инсайдера/споров.
  Метод прежний: реальные сервисы (`build-services.ts`) + тест-БД + наблюдения; перед каждым тестом — бизнес-правило + 🟢/🔴.
