...
- **⏭️ ТОЧНЫЙ СЛЕДУЮЩИЙ ШАГ (продолжить отсюда): Фаза 6 В ПРОЦЕССЕ — сеть (bootstrap-smoke) ГОТОВА, дальше резать первый цикл.** Сеть: `bootstrap.int-spec.ts` реально поднимает `AppModule.compile()` (сторож против битых `forwardRef`). Прогон: `yarn test:int` = core 18/18 + boot 1/1 (изолированы в разные процессы — иначе BullMQ-воркеры bootstrap флакуют F1-гонку). **✅ Разорваны users↔auth (шаг 2, @Global-избыточность) и contests↔contests-jobs (шаг 3, producer ContestJobsService переехал в contests → jobs→contests односторонний).** Оба: core 18/18 + boot 1/1. **Осталось 2 цикла:** bot↔channels, толстый bot↔contests (хаб TelegramService — последним). Приёмы: (а) избыточный импорт @Global-модуля; (б) перенос producer/односторонней зависимости в правильный домен. Правило шага: после разрыва И core 18/18, И boot 1/1. **Не закоммичен:** разрез contests↔jobs (шаг 3) — закоммитить (шаги 1,2 уже в origin).
