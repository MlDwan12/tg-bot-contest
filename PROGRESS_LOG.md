...
- **⏭️ ТОЧНЫЙ СЛЕДУЮЩИЙ ШАГ (продолжить отсюда): Фаза 4 — Sentry (🟡)** по порядку плана (⓿→1 initData[отложена]→2 F1✅→3 аудит✅→**4 Sentry**→5 bcrypt→6 TelegramTransport→7 быстрые победы). Sentry-пакеты уже в deps (`@sentry/node`, `@sentry/tracing`) — проверить, инициализирован ли, настроить перехват ошибок/иначе. Детали согласовать с пользователем перед кодом.
  - Альтернативы, если Sentry не в приоритете: **Фаза 7 (быстрые победы 🟢)** — напр. типизировать `findByIdWithRelations: Promise<any>` (убирает флот `no-unsafe-*` в `contests.service.ts`); или **MANUAL-очистка** победителей тоже в аудит (сейчас пишем только назначение непустого списка, `contests.service.ts:294/343` — clear без следа).
