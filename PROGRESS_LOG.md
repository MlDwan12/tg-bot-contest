...
- **⏭️ ТОЧНЫЙ СЛЕДУЮЩИЙ ШАГ (продолжить отсюда): Фаза 6 В ПРОЦЕССЕ — сеть (bootstrap-smoke) ГОТОВА, дальше резать первый цикл.** Сеть: `bootstrap.int-spec.ts` реально поднимает `AppModule.compile()` (сторож против битых `forwardRef`). Прогон: `yarn test:int` = core 18/18 + boot 1/1 (изолированы в разные процессы — иначе BullMQ-воркеры bootstrap флакуют F1-гонку). **Первый цикл на разрыв — рекомендация: `users↔auth` (самый изолированный), НЕ толстый bot↔contests.** Правило шага: после разрыва И core 18/18, И boot 1/1 зелёные (boot ловит битый DI, которого 18 тестов не видят). Файлы Фазы 6 (`bootstrap.int-spec.ts`, `.env.test`, `jest-integration.json`, `package.json` split) ещё НЕ закоммичены — закоммитить net перед первым разрезом.
