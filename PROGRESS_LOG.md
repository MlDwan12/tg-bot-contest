...
- **⏭️ ТОЧНЫЙ СЛЕДУЮЩИЙ ШАГ (продолжить отсюда): Фаза 9 В ПРОЦЕССЕ — единый паттерн репо (токен+интерфейс). User + Channel + Contest + ContestParticipate СДЕЛАНЫ, остался ФИНАЛЬНЫЙ ContestWinner.** Эталон (память [[repository-pattern-convention]]): один `XRepository implements IXRepository` + один `IXRepository` + токен `X_REPOSITORY` (`tokens.ts`) + провайдер `{ provide, useClass }` + инъекция `@Inject(X_REPOSITORY) repo: IXRepository` (интерфейс — `import type`!). **Осталось слить read/write у 1 агрегата:** ContestWinner (`CONTEST_WINNER_READ/WRITE_REPOSITORY` → `CONTEST_WINNER_REPOSITORY`). Инъектор — `ContestWinnerService` (read+write); + `contests.module` (2 `useExisting` → 1 `useClass`); + харнесс `build-services.ts` (`winnerRead`/`winnerWrite` → `winner`, позиционный конструктор `ContestWinnerService` × 2 места: `buildWinnerService`/`buildLifecycleService`). **ВАЖНО: `ContestWinnerAuditWriteRepository` — write-only, НЕ трогать** (отдельный аудит-агрегат, остаётся как есть). Схема: (1) слить 2 репо-класса → `contest-winner.repository.ts`; (2) слить 2 интерфейса → по факту; (3) токен; (4) провайдер `useClass`; (5) инъекция в winner-сервисе; (6) удалить старые файлы, barrels. Правило шага: build чист + core 18/18 + boot 1/1. **Метод тестов:** `yarn test:db:up` если контейнеры погашены, затем `yarn test:int`. **Всё закоммичено и запушено** (User=3bf6c1f; Channel+Contest=98ef1c3; ContestParticipate — новый коммит). **После ContestWinner — Фаза 9 ЗАКРЫТА.**
...
- [~] Фаза 9 — схлопнуть CQRS-репозитории 🟢 (В ПРОЦЕССЕ: эталон токен+интерфейс выбран, User+Channel+Contest+Participate слиты; остался ФИНАЛЬНЫЙ Winner)
